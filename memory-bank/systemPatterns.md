...
- **VisionMaster 桥接进程模式** ✅（v1.24）：跨运行时 SDK 隔离——VM SDK 为 .NET Framework 程序集（GAC），net10.0 主程序无法直引，由 `tools/VmVisionBridge`（net48 x64）独立进程承载 SDK：① 主程序与桥接**共享同一份协议源码**（`Common/VmBridge/VmBridgeProtocol.cs` 双侧 link 编译，杜绝漂移）；② 二进制帧协议（命令 1B + 长度 4B LE + payload；响应 = key=value 头部 + `\n\n` 分隔 + PNG 二进制，中文 Base64）；③ 主程序 `VisionMasterBridgeInspectionService` 懒启动桥接进程 + `NamedPipeClientStream` 收发（启动/加载/运行分级超时），进程异常退出下次调用自动重启；④ SDK 对象（VmSolution/VmProcedure/ImageBaseData）全封装在桥接进程内，主程序只见状态与 PNG 字节；⑤ 全部失败走 `Result.Fail` 不抛 UI 异常
