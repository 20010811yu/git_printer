...
- 📁 当前阶段：**多页面应用 + 单元测试基础设施已开发完成并可运行**
  ...
  - ✅ PLC 抽屉物料接入（v1.12）：连续读取 M1000 起 19 个 bool（下标 1~18 对应抽屉，true=有料），变化推送驱动抽屉状态灯；PLC 为物料唯一真值源（Mock 随机监控停用）
   ...
   - ✅ VM 方案路径切换 VisionTesting.sol（v1.25）：生产方案路径 Program.cs DI 单点维护（`D:\Printer\VisionTesting.dll`，明文 .sol 内容、扩展名不限；v1.30 曾加密、v1.33 起移除加密直载），路径由 Service 配置提供、ViewModel 不感知；接口 `LoadSolutionAsync(string? = null)` 默认参数化
