...
# 方案加载：明文 .sol 直载（v1.33 起不加密；v1.30 加密机制与 tools/SolutionEncryptor 已删除）
...
6a. **AntdUI Input 绑定语义（v1.32 实证）**：`AntdUI.Input` 继承 IControl(Control)，`Text`/`ReadOnly`/`TextAlign` 与原生 TextBox 同名可直接迁移；其 `SetText` 会触发 `Control.OnTextChanged`（IL 实证），WinForms `DataBindings` 双向绑定可行——但绑定仅在控件挂到**已显示窗体**（有句柄 + BindingContext）后激活，View 绑定测试必须建宿主 Form 并 Show（详 ERR-036，同源 ERR-029）
