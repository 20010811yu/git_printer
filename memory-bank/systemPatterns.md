...
├── tests\UiTopMachine.Tests\   # 单元测试（xUnit，net10.0-windows；79 用例覆盖命令/三态/xlsx往返/VM业务/编号查重/新建配方轮转/行序整理）
└── memory-bank\           # 项目记忆文档
```
...
8. ✅ 单元测试基础设施（tests/UiTopMachine.Tests：xUnit + .slnx + .gitignore，79 用例全绿；每次任务修改功能必须配套测试并全绿，2026-09-03 固化，详 techContext.md「测试工作流」）
