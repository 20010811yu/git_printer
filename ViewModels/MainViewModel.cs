...
namespace UiTopMachine.ViewModels
{
    ...
    public class MainViewModel : ObservableObject, IPanelStatusPublisher
    {
        ...
        /// PLC 侧 D4000 起为 16 位 INT 且地址连续（用户确认），发送时把编号数组一次批量写入
        /// Write("4000", array)，元素按顺序落 4000 起连续寄存器（不含配方值）
        /// </summary>
        ...
        /// PLC 侧 D4000 起为 16 位 INT 连续地址，元素按顺序落连续寄存器，不写配方值；
        ...
        private async Task SendAllRecipesAsync()
        {
            ...
            try
            {
                ...
                // 一次批量写入编号数组：PLC 侧 D4000 起为 16 位 INT 连续地址，元素按顺序落连续寄存器
